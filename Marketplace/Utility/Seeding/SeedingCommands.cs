    ...
    public static class SeedingCommands
    {
        ...
        public static async Task<HandleResult> TryHandleAsync(string[] args, WebApplication app, CancellationToken ct = default)
        {
            ...
            if (isUserList)
            {
                ...
                int take = 20;
                if (takeRaw != null && int.TryParse(takeRaw, out var parsedTake)) take = Math.Clamp(parsedTake, 1, 50);
                using var scope = app.Services.CreateScope();
                var userSeeder = scope.ServiceProvider.GetRequiredService<UserSeeder>();
                var users = await userSeeder.ListUsersAsync(search, take, ct);
                if (users.Count == 0)
                {
                    Console.WriteLine(string.IsNullOrWhiteSpace(search) ? "No users found." : $"No users found for '{search}'.");
                }
                else
                {
            ...
            if (isDemo)
            {
                ...
                if (countRaw != null && int.TryParse(countRaw, out var parsedCount)) count = Math.Clamp(parsedCount, 1, 200);
                var demoEqRaw = ParseArgValue(args, "seed:demo", "seed-demo", "seeddemo", "demo");
                if (demoEqRaw != null && int.TryParse(demoEqRaw, out var parsedDemo)) count = Math.Clamp(parsedDemo, 1, 200);
                else
                {
                    ...
                    if (demoEq != null && demoEq.Contains('=') && int.TryParse(demoEq.Split(new[] { '=', ':' }, 3).Last(), out var parsed2)) count = Math.Clamp(parsed2, 1, 200);
                    else if (demoEq != null && demoEq.Contains(':') && int.TryParse(demoEq.Split(':').Last(), out var parsed3)) count = Math.Clamp(parsed3, 1, 200);
                }
        ...
        private static string? ParseArgValue(string[] args, params string[] names)
        {
            ...
            {
                var raw = args[i].Trim();
                if (IsCommandToken(raw)) continue;
                ...
                else
                {
                    ...
                    if (i + 1 < args.Length)
                    {
                        ...
                        // next is value if it doesn't look like a flag/command
                ...
                {
                    if (!string.IsNullOrWhiteSpace(value)) return value.Trim().Trim('"').Trim('\'');
                    if (eqIdx >= 0) return "";
